...
void MemStore::notify_subscribers(const string& actor_id, const string& key, const string& value){
...
uint64_t MemStore::subscribe(const string& actor_id, SubCallback callback) {
    return subscription_system_.subscribe(actor_id, 
        [callback, actor_id](const string& key, const string& value) {
            callback(actor_id, key, value);
        });
}

bool MemStore::unsubscribe(const string& actor_id, uint64_t sub_id) {
    return subscription_system_.unsubscribe(actor_id, sub_id);
}

void MemStore::write_behind_append(const string& actor_id, const string& key, const string& value) {
